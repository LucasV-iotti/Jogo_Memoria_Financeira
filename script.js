(function(){
  ...
  function init(){
    try{
      ...
      // ===== Estado =====
      const state={ player:{name:''}, deck:[], started:false, previewActive:false, previewInterval:null,
        answeredThemes:new Set(), correctAnswers:0, pairsFound:0, selected:[], lockBoard:false,
        score(){ return this.correctAnswers + this.pairsFound*SCORE_PAIR_BONUS; }
      };
      ...
      function createDeck(){
        ...
        const deck=[];
        for(const q of QUESTIONS){ for(let i=0;i<2;i++){ deck.push({ id:q.id+'_'+i, theme:q.theme, label:map[q.theme].label, emoji:map[q.theme].emoji, flipped:false, matched:false, el:null }); } }
        return shuffle(deck)
      }
      function renderBoard(){
        const board=$('#board'); if(!board) return; board.innerHTML='';
        state.deck.forEach(card=>{
          const it=document.createElement('button'); it.className='card3d'; it.setAttribute('aria-label',`Carta ${card.label}`);
          it.innerHTML=`<div class="inner"><div class="face front"><div class="card-emoji">✨</div><div class="card-theme">Memorize</div></div><div class="face back"><div class="card-emoji">${card.emoji}</div><div class="card-theme">${card.label}</div></div></div>`;
          it.addEventListener('click',()=>onCardClick(card)); card.el=it; board.appendChild(it);
        });
      }
      ...
      // ===== Navegação =====
      function goto(id){ $$('.screen').forEach(s=>s.classList.remove('active')); const el=document.querySelector(id); if(el) el.classList.add('active') }

      // ===== Setup =====
      function setupGame(){
        state.deck=createDeck(); state.started=false; state.previewActive=false; state.answeredThemes=new Set();
        state.correctAnswers=0; state.pairsFound=0; state.selected=[]; state.lockBoard=false;
        renderBoard(); updateHUD();
        const hint=$('#hint'); if(hint) hint.textContent='Dica: você pode usar a pré‑visualização (20s) para memorizar as posições.';
        const bprev=$('#btn-preview'), bstart=$('#btn-start'); if(bprev) bprev.disabled=false; if(bstart) bstart.disabled=false;
        $('#preview-timer')?.classList.add('hidden');
      }
      ...
      // ===== Pré-visualização =====
      function startPreview(){ if(state.started||state.previewActive) return; state.previewActive=true; $('#btn-preview').disabled=true; $('#btn-start').disabled=true; $('#hint').textContent='Pré‑visualização em andamento: memorize as posições!'; state.deck.forEach(c=>setCardFlip(c,true)); let t=20; const badge=$('#preview-timer'); badge.classList.remove('hidden'); badge.textContent=t+'s'; state.previewInterval=setInterval(()=>{ t--; badge.textContent=t+'s'; if(t<=0){ clearInterval(state.previewInterval); endPreview(); } },1000) }
      ...
      function onCardClick(card){
        if(!state.started){ toast('Clique em "Iniciar jogo" para começar.'); return }
        if(state.previewActive||state.lockBoard||card.matched||card.flipped) return;
        setCardFlip(card,true); state.selected.push(card);
        if(state.selected.length===2){
          state.lockBoard=true; const [c1,c2]=state.selected;
          if(c1.theme===c2.theme && c1.id!==c2.id){ setTimeout(()=>{ setCardMatched(c1); setCardMatched(c2); state.pairsFound++; updateHUD(); toast('Par encontrado! +'+SCORE_PAIR_BONUS+' pontos'); state.selected=[]; openQuestionModal(c1.theme); },350) }
          else { setTimeout(()=>{ setCardFlip(c1,false); setCardFlip(c2,false); state.selected=[]; state.lockBoard=false },700) }
        }
      }
      ...
      function openQuestionModal(theme){
        if(state.answeredThemes.has(theme)){ state.lockBoard=false; if(state.pairsFound===QUESTIONS.length && state.answeredThemes.size===QUESTIONS.length){ endGame() } return }
        const q = QUESTIONS.find(x=>x.theme===theme); if(!q){ state.lockBoard=false; return }
        qTitle.textContent = q.title; qText.textContent = q.text; qOptions.innerHTML='';
        q.options.forEach(opt=>{ const b=document.createElement('button'); b.type='button'; b.className='option-btn'; b.innerHTML=`<strong>${opt.key})</strong> ${opt.text}`; b.addEventListener('click',()=>handleAnswer(q,opt.key)); qOptions.appendChild(b) });
        try{ qModal.showModal() }catch{ qModal.setAttribute('open','open') }
      }
      function handleAnswer(q, chosen){
        qOptions.querySelectorAll('button').forEach(b=>b.disabled=true);
        const correct=(chosen===q.correct);
        qOptions.querySelectorAll('button').forEach(b=>{ const k=b.textContent.trim().charAt(0); if(k===q.correct) b.classList.add('correct'); if(k===chosen && !correct) b.classList.add('wrong') })
        if(correct){ state.correctAnswers++; toast('Resposta correta! +1 ponto') } else { toast('Resposta incorreta.') }
        state.answeredThemes.add(q.theme); updateHUD();
        setTimeout(()=>{ try{ qModal.close() }catch{} state.lockBoard=false; if(state.pairsFound===QUESTIONS.length && state.answeredThemes.size===QUESTIONS.length){ setTimeout(endGame,400) } },450)
      }

      // ===== Fim =====
      function endGame(){
        $('#final-name').textContent = state.player.name || '—';
        $('#final-correct').textContent = state.correctAnswers;
        $('#final-pairs').textContent = state.pairsFound;
        $('#final-score').textContent = state.score();
        const fb = buildFeedback(state.correctAnswers, state.pairsFound, state.score());
        $('#final-feedback').textContent = fb;
        goto('#final-screen')
      }
      function buildFeedback(c,p,s){
        if(c===6 && p===6) return 'Perfeito! Você dominou as perguntas e a memória espacial. Parabéns, comandante!';
        if(c>=5) return 'Excelente! Ótimo domínio do conteúdo e boa estratégia de memória.';
        if(c>=3) return 'Bom resultado! Continue praticando para evoluir ainda mais.';
        return 'Você está no caminho! Revisar o conteúdo e tentar novamente vai turbinar seu desempenho.';
      }

      // ===== Bindings =====
      const form=$('#player-form');
      if(form){ form.addEventListener('submit', (e)=>{ e.preventDefault(); const name=$('#player-name')?.value.trim()||''; if(!name){ toast('Por favor, informe seu nome.'); return } state.player.name=name; $('#hud-name').textContent=name; setupGame(); goto('#game-screen'); return false }) }
      $('#btn-preview')?.addEventListener('click', startPreview);
      $('#btn-start')?.addEventListener('click', startGame);
      $('#btn-restart')?.addEventListener('click', ()=>{ setupGame(); goto('#game-screen') });

    }catch(err){ console.error('[Jogo] Erro:', err); alert('Erro ao iniciar o jogo. Abra o Console (F12) e envie a mensagem exibida.') }
  }
})();
